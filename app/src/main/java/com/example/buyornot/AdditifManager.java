...
public class AdditifManager {
    ...
    public long addAdditif(Additif additif) {
        ...
        values.put(KEY_LIBELLE_ADDITIF, additif.getCat_libelle());
    ...
    public int modAdditif(Additif additif) {
        ...
        String[] whereArgs = {additif.getCat_id()+""};
    ...
    public int supAdditif(Additif additif) {
        ...
        String where = KEY_ID_ADDITIF+" = ?";
        String[] whereArgs = {additif.getCat_libelle()+""};
    ...
    public Additif getAdditif(int id) {
        // Retourne l'additif dont l'id est passé en paramètre

        Additif p = new Additif(1, "sucre");

        Cursor c = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + KEY_ID_ADDITIF + "=" + id, null);

        if (c.moveToFirst()) {
            p.setAdd_id(c.getInt(c.getColumnIndex(KEY_ID_ADDITIF)));
            p.setAdd_libelle(c.getString(c.getColumnIndex(KEY_LIBELLE_ADDITIF)));
            c.close();
        }
        return p;
    }
